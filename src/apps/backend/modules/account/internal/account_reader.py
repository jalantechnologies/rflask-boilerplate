...
class AccountReader:
    ...
    def get_account_by_username(*, username: str) -> AccountModel:
        ...
        if not account.active:
            raise AccountNotFoundError(f"Account with username:: {username}, queued for deletion")
    ...
    def check_username_not_exist(*, params: CreateAccountByUsernameAndPasswordParams) -> None:
        ...
        if account:
            ...
            if account.active:
                ...
                raise AccountWithUserNameExistsError(f"Account for username:: {params.username}, not been deleted yet")

    @staticmethod
    def get_account_by_phone_number_optional(*, phone_number: PhoneNumber) -> Optional[Account]:
        account_data = AccountRepository.collection().find_one({"phone_number": phone_number})
        if account_data is None:
            return None
        ...
        if not account.active:
            raise AccountNotFoundError(f"Account with phone number:: {phone_number}, queued for deletion")
